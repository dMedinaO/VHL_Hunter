...
class responseTraining(object):
    ...
    def estimatedMetricsPerformanceMultilabels(self, dataInput, dataClass):
        ...
        accuracyResponse = cross_val_score(
            self.clf, dataInput, dataClass, cv=self.validation, scoring="accuracy"
        )
        accuracyValue = np.mean(accuracyResponse)

        f1_weightedResponse = cross_val_score(
            self.clf, dataInput, dataClass, cv=self.validation, scoring="f1_weighted"
        )
        f1_weighted = np.mean(f1_weightedResponse)

        recall_weightedResponse = cross_val_score(
            self.clf,
            dataInput,
            dataClass,
            cv=self.validation,
        ...
        precision_weightedResponse = cross_val_score(
            self.clf,
            dataInput,
            dataClass,
            cv=self.validation,
    ...
    def estimatedMetricsPerformance(self, dataInput, dataClass):
        ...
        for element in self.ListScore:
            scores = cross_val_score(
                self.clf, dataInput, dataClass, cv=self.validation, scoring=element
            )
